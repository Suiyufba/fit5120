...
- Route planning now treats `local-admin` as a valid local session and uses an `advanced` fallback user profile,
  so Plan Route works for admin instead of querying PostgreSQL with the string id `local-admin`.
